...
library Hero {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "ggg", name: "Hero", typeId: RESOURCE_TABLE });`
  ResourceId constant _tableId = ResourceId.wrap(0x746267676700000000000000000000004865726f000000000000000000000000);
   ...
   */
  function get(uint256 id) internal view returns (HeroData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = bytes32(uint256(id));
   ...
   * @notice Get the full data.
   */
  function _get(uint256 id) internal view returns (HeroData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = bytes32(uint256(id));
   ...
   */
  function set(uint256 id, uint256 health, uint256 attack) internal {
    bytes memory _staticData = encodeStatic(health, attack);
   ...
   * @notice Set the full data using individual values.
   */
  function _set(uint256 id, uint256 health, uint256 attack) internal {
    bytes memory _staticData = encodeStatic(health, attack);
   ...
   */
  function set(uint256 id, HeroData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.health, _table.attack);
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(uint256 id, HeroData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.health, _table.attack);
   ...
   */
  function decodeStatic(bytes memory _blob) internal pure returns (uint256 health, uint256 attack) {
    health = (uint256(Bytes.getBytes32(_blob, 0)));

    attack = (uint256(Bytes.getBytes32(_blob, 32)));
  }
  ...
  function decode(
    ...
    EncodedLengths,
    bytes memory
  ) internal pure returns (HeroData memory _table) {
    (_table.health, _table.attack) = decodeStatic(_staticData);
  }
   ...
   */
  function encodeStatic(uint256 health, uint256 attack) internal pure returns (bytes memory) {
    return abi.encodePacked(health, attack);
  }
   ...
   */
  function encode(uint256 health, uint256 attack) internal pure returns (bytes memory, EncodedLengths, bytes memory) {
    bytes memory _staticData = encodeStatic(health, attack);
