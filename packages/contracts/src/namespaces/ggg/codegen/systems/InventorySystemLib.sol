...
// equivalent to WorldResourceIdLib.encode({ typeId: RESOURCE_SYSTEM, namespace: "ggg", name: "InventorySystem" }))
InventorySystemType constant inventorySystem = InventorySystemType.wrap(
  0x73796767670000000000000000000000496e76656e746f727953797374656d00
);
...
library InventorySystemLib {
  ...
  function move(InventorySystemType self, Direction direction) internal {
    return CallWrapper(self.toResourceId(), address(0)).move(direction);
  }

  function move(CallWrapper memory self, Direction direction) internal {
    // if the contract calling this function is a root system, it should use `callAsRoot`
    if (address(_world()) == address(this)) revert InventorySystemLib_CallingFromRootSystem();

    bytes memory systemCall = abi.encodeCall(_move_Direction.move, (direction));
    self.from == address(0)
      ? _world().call(self.systemId, systemCall)
      : _world().callFrom(self.from, self.systemId, systemCall);
  }

  function move(RootCallWrapper memory self, Direction direction) internal {
    bytes memory systemCall = abi.encodeCall(_move_Direction.move, (direction));
